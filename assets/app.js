...
el.style.transition = `opacity 0.65s cubic-bezier(0.16,1,0.3,1) ${i * 0.055}s, transform 0.65s cubic-bezier(0.16,1,0.3,1) ${i * 0.055}s`;
requestAnimationFrame(() => {
...
const size = 6 - i;
dot.style.cssText = `position:fixed;width:${size}px;height:${size}px;border-radius:50%;background:rgba(200,169,90,${0.5 - i * 0.07});pointer-events:none;z-index:999999;transform:translate(-50%,-50%);mix-blend-mode:screen;transition:opacity 0.3s;`;
...
card.appendChild(sh);
on(card, ‘mouseenter’, () => { sh.style.transition = ‘left 0.7s ease’; sh.style.left = ‘160%’; });
...
rpl.style.cssText = `position:absolute;width:4px;height:4px;background:rgba(200,169,90,0.4);border-radius:50%;left:${e.clientX - r.left}px;top:${e.clientY - r.top}px;transform:translate(-50%,-50%) scale(0);animation:rippleOut 0.55s ease forwards;pointer-events:none;z-index:10;`;
