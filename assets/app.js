...
card.style.transform = 'perspective(800px) rotateX(' + tx + 'deg) rotateY(' + ty + '
});
...
sh.style.width = '60%';
sh.style.height = '100%';
sh.style.background = 'linear-gradient(90deg,transparent,rgba(200,169,90,0.05),rgba(2
...
style.textContent = '.h-reveal{position:relative;}.h-reveal::after{content:"";position:ab
...
if (PREFERS_REDUCED || IS_COARSE) return;
var style = document.createElement('style');
style.textContent = '.nav__link{position:relative;}.nav__link::after{content:"";position:
document.head.appendChild(style);
}
/* ===== INIT ===== */
